...
extension Coordinator {
    func updateCoordinator(arView: ARView, focusEntity: FocusEntity) {
        ...
        focusTimer = Timer.scheduledTimer(
            withTimeInterval: Constants.focusUpdateInterval,
            repeats: true
        ) { [weak focusEntity] _ in
...
extension Coordinator {
    ...
    @objc func handlePinch(_ gesture: UIPinchGestureRecognizer) {
        ...
        modelEntity.clampScale(
            currentScale, minScale: initialScale * Constants.minScaleFactor,
            maxScale: initialScale * Constants.maxScaleFactor
        )
    }
